...
export async function registerDemoRoutes(
  ...
  app.post(
    ...
    async (request) => {
      ...
      if (!deps.config.DEMO_MODE_ENABLED || !deps.config.DEMO_CAMPAIGN)
        throw new Error("Demo Mode is unavailable");
      const campaign = await deps.pool.query<{ eligible: boolean }>(
        "SELECT state = 'active' AND terminal_reason = 'none' AND hard_expiry > clock_timestamp() AS eligible FROM campaign_projections WHERE campaign = $1",
        [deps.config.DEMO_CAMPAIGN],
      );
      if (!campaign.rows[0]?.eligible)
        throw new Error("the predetermined demo campaign is not active");
      ...
      return {
        id: capability,
        campaign: deps.config.DEMO_CAMPAIGN,
      ...
      try {
        ...
        const session = await client.query<{ campaign: string; step: number }>(
         ...
         WHERE session_hash = $1 AND origin = $2 AND expires_at >= clock_timestamp()
           AND request_count < 20 AND (last_request_at IS NULL OR last_request_at <= clock_timestamp() - interval '1 second')
  ...
  app.post(
    ...
    async (request) => {
      const origin = requireWriteOrigin(request, deps.config);
      const { id } = sessionParams.parse(request.params);
      const result = await deps.pool.query<{ campaign: string }>(
